...
export default function Home() {
  ...
  // ★ ИЗМЕНЕНИЕ 1: добавлены поля phone и telegram
  ...
  // ★ ИЗМЕНЕНИЕ 2: Lightbox — кнопка "Напиши мне" открывает DlModal поверх фото (не закрывает lightbox)
  const Lightbox = () => {
    const arr = getDisplayPhotos()
    ...
    return (
      <div style={{position:'fixed',inset:0,zIndex:1000,background:'rgba(0,0,0,0.97)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}>
        ...
        <img src={lightbox.url} alt={lightbox.title} draggable={false}
          style={{maxWidth:'calc(100vw - 140px)',maxHeight:'calc(100vh - 160px)',width:'auto',height:'auto',objectFit:'contain',pointerEvents:'none',userSelect:'none',borderRadius:2}}/>
        <div style={{position:'absolute',bottom:0,left:0,right:0,padding:'1.5rem 2rem',background:'linear-gradient(to top,rgba(0,0,0,0.85),transparent)',display:'flex',alignItems:'flex-end',justifyContent:'space-between',zIndex:2}}>
          ...
          <div style={{display:'flex',gap:10,flexShrink:0,marginLeft:16}}>
            ...
            <button onClick={e=>{e.stopPropagation();setDlPhoto(lightbox);setShowDlModal(true)}}
              style={{fontSize:'0.75rem',padding:'8px 18px',borderRadius:2,border:`1px solid ${C}`,background:'transparent',color:C,cursor:'pointer'}}>
              ✉ Напиши мне
            </button>
            {isAdmin&&(
  ...
  if (view==='gallery') {
    ...
    return (
      <div style={{minHeight:'100vh',background:BG,color:TXT,fontFamily:"'Jost',sans-serif",fontWeight:300,overflowY:'auto'}}>
        ...
        {lightbox&&<Lightbox/>}
        {/* ★ ИЗМЕНЕНИЕ 3: DlModal рендерится ПОСЛЕ Lightbox с zIndex 1100 — всегда поверх фото */}
    ...
    <div style={{minHeight:'100vh',background:BG,color:TXT,fontFamily:"'Jost',sans-serif",fontWeight:300,overflowY:'auto'}}>
      ...
      <div style={{maxWidth:900,margin:'0 auto',padding:isMobile?'calc(56px + 1.5rem) 1.25rem 3rem':'calc(64px + 3rem) 3rem 5rem',display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?'2rem':'5rem',alignItems:'start'}}>
        ...
        <div>
          ...
          {/* ★ ИЗМЕНЕНИЕ 4: отображаем phone и telegram если заполнены */}
          {[
            ['@', about.email],
            ['ig', about.ig],
            about.phone ? ['☎', about.phone] : null,
            about.telegram ? ['tg', about.telegram] : null,
            ['✦', about.city]
  ...
  // ★ ИЗМЕНЕНИЕ 5: DlModal — zIndex 1100 (поверх lightbox), показывает контакты сразу карточками
  function DlModal() {
    const subj=encodeURIComponent('Запрос на фото: '+dlPhoto.title)
    const body=encodeURIComponent('Здравствуйте!\n\nХочу запросить фото «'+dlPhoto.title+'».\n\nЦель: \n\nС уважением,')
    const tgHandle = about.telegram
      ? (about.telegram.startsWith('@') ? about.telegram.slice(1) : about.telegram)
      : (about.ig?.startsWith('@') ? about.ig.slice(1) : about.ig)
    return (
      <div style={{...MB, zIndex:1100}} onClick={e=>e.target===e.currentTarget&&setShowDlModal(false)}>
        <div style={{...MBX,textAlign:'center'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:10,marginBottom:16,textAlign:'left'}}>
            {about.email&&(
              <a href={`mailto:${about.email}?subject=${subj}&body=${body}`} target="_blank"
                style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',borderRadius:2,background:'rgba(200,169,110,0.08)',border:'1px solid rgba(200,169,110,0.4)',color:TXT,textDecoration:'none',fontSize:'0.78rem'}}>
                <span style={{color:C,fontSize:'1rem',width:20,flexShrink:0,textAlign:'center'}}>@</span>
                <div>
                  <div style={{fontSize:'0.62rem',letterSpacing:'0.15em',textTransform:'uppercase',color:C,marginBottom:2}}>Email</div>
                  <div>{about.email}</div>
                </div>
              </a>
            )}
            {tgHandle&&(
              <a href={`https://t.me/${tgHandle}`} target="_blank"
                style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',borderRadius:2,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(232,226,217,0.15)',color:TXT,textDecoration:'none',fontSize:'0.78rem'}}>
                <span style={{color:C,fontSize:'0.85rem',width:20,flexShrink:0,textAlign:'center',fontWeight:500}}>tg</span>
                <div>
                  <div style={{fontSize:'0.62rem',letterSpacing:'0.15em',textTransform:'uppercase',color:C,marginBottom:2}}>Telegram</div>
                  <div>{about.telegram||about.ig}</div>
                </div>
              </a>
            )}
            {about.phone&&(
              <a href={`tel:${about.phone}`}
                ...
                <span style={{color:C,fontSize:'1rem',width:20,flexShrink:0,textAlign:'center'}}>☎</span>
                <div>
                  <div style={{fontSize:'0.62rem',letterSpacing:'0.15em',textTransform:'uppercase',color:C,marginBottom:2}}>Телефон</div>
                  <div>{about.phone}</div>
                </div>
              </a>
            )}
  ...
  // ★ ИЗМЕНЕНИЕ 6: AboutEditModal — добавлены поля Telegram и Телефон
      ...
      <div style={MB} onClick={e=>e.target===e.currentTarget&&setShowAboutEdit(false)}>
        <div style={MBX}>
          ...
          {[
            ...
            ['role','Подзаголовок'],
            ['email','Email'],
            ['ig','Instagram (например @username)'],
            ['telegram','Telegram (например @username)'],
