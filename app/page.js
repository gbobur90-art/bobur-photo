        ...
        <div style={{...MBX,width:560,maxWidth:'100%'}}>
          ...
          <div style={{marginBottom:'1rem'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:6,maxHeight:200,overflowY:'auto'}}>
              {photos.map(p=>(
                <div key={p.id} onClick={()=>setSel(prev=>prev.includes(p.id)?prev.filter(x=>x!==p.id):[...prev,p.id])} style={{aspectRatio:'1',position:'relative',cursor:'pointer',border:sel.includes(p.id)?`2px solid ${C}`:'2px solid transparent',borderRadius:2,overflow:'hidden',background:'#1a1a1a'}}>
